...
func main() {
	...
	mux.Handle("POST /app/savings/calculate", config.AuthMiddleware(http.HandlerFunc(config.HandlerSavingsCalculateGet)))
	mux.Handle("POST /app/loans/calculate", config.AuthMiddleware(http.HandlerFunc(config.HandlerLoansCalculateGet)))
